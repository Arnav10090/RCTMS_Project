  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <DataCard title="DM Water System" icon={Droplets} variant="primary">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <div className="text-xs text-muted-foreground">Flow Rate</div>
                <div className="font-mono font-semibold text-primary">
                  {systemData.dmWater.flowRate} L/min
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Pressure</div>
                <div className="font-mono font-semibold">
                  {systemData.dmWater.pressure} bar
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Conductivity</div>
                <div className="font-mono font-semibold">
                  {systemData.dmWater.conductivity} µS/cm
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">pH Level</div>
                <div className="font-mono font-semibold">
                  {systemData.dmWater.pH}
                </div>
              </div>
            </div>
            <GaugeDisplay
              label="Temperature"
              value={systemData.dmWater.temperature}
              min={15}
              max={35}
              unit="°C"
              thresholds={{
        ...
        <DataCard title="Heat Exchanger Network" icon={Thermometer}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <div className="text-xs text-muted-foreground">Inlet Temp</div>
                <div className="font-mono font-semibold text-warning">
                  {systemData.heatExchanger.inletTemp}°C
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Outlet Temp</div>
                <div className="font-mono font-semibold text-success">
                  {systemData.heatExchanger.outletTemp}°C
                </div>
              </div>
            </div>
            <GaugeDisplay
              label="Heat Transfer Efficiency"
              value={systemData.heatExchanger.efficiency}
              min={70}
              max={100}
              unit="%"
              thresholds={{
            ...
            />
            <div>
              <div className="text-xs text-muted-foreground">Bypass Position</div>
              <div className="font-mono font-semibold">
                {systemData.heatExchanger.bypassPosition}%
              </div>
            </div>
          </div>
        </DataCard>

        <DataCard title="Filtration System" icon={Filter}>
          <div className="space-y-4">
            <StatusIndicator 
              status="active" 
              label="Backwash Status" 
              value={systemData.filtrationSystem.backwashStatus}
            />
            <div className="space-y-3">
              <div>
                <div className="text-xs text-muted-foreground">Pressure Differential</div>
                <div className="font-mono font-semibold">
                  {systemData.filtrationSystem.pressureDiff} bar
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Replacement Due</div>
                <div className="font-mono font-semibold text-warning">
                  {systemData.filtrationSystem.replacementDue} hours
                </div>
              </div>
            </div>
